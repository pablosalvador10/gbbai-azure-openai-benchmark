...
def combine_logs_to_csv(
    ...
    run_summaries = [
        extract_run_info_from_log_path(log_file, stat_extraction_point)
...
def extract_run_info_from_log_path(
    log_file: str, stat_extraction_point: str
) -> Optional[dict]:
    ...
    with open(log_file) as f:
        for line in f.readlines():
            ...
            if "All data samples: " in line:
                raw_samples = line.split("All data samples: ")[
                    -1
                ]  # Do not load - output as string
    if not run_args:
        logging.error(
