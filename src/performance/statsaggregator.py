...
class _StatsAggregator(threading.Thread):
    ...
    def dump_raw_call_stats(self):
        ...
        samples = {
            "request_timestamps": [
                round(val, 4) for val in self.request_timestamps._values()
            ],
            "request_latency": [
                round(val, 4) for val in self.request_latency._values()
            ],
            "call_tries": self.call_tries._values(),
            "response_latencies": [
                round(val, 4) for val in self.response_latencies._values()
            ],
            "first_token_latencies": [
                round(val, 4) for val in self.first_token_latencies._values()
            ],
            "token_latencies": [
                round(val, 5) for val in self.token_latencies._values()
            ],
    ...
    def _dump(self):
        with self.lock:
            ...
            gen_per_minute = (
                round(
                    60.0 * np.sum(self.generated_tokens._values()) / dynamic_window, 0
                )
            ...
            gen_tpr_10th = (
                ...
                if self.generated_tokens._len() > 1
                else "n/a"
            )
            gen_tpr_90th = (
