...
class OAIRequester:
    ...
    async def _call(
        self, session: aiohttp.ClientSession, body: dict, stats: RequestStats
    ):
        headers = {
        ...
        while (
            stats.calls == 0
            or time.time() - stats.request_start_time < MAX_RETRY_SECONDS
        ):
            stats.calls += 1
            ...
            if self.backoff and RETRY_AFTER_MS_HEADER in response.headers:
                try:
                    ...
                    logging.warning(
                        f"unable to parse retry-after header value: {UTILIZATION_HEADER}={retry_after_str}: {e}"
                    )
                    # fallback to backoff
                    break
            else:
        ...
        if response.status != 200 and response.status != 429:
            logging.warning(
                f"call failed: {REQUEST_ID_HEADER}={response.headers[REQUEST_ID_HEADER]} {response.status}: {response.reason}"
            )
        if self.backoff:
    ...
    async def _handle_response(
        self, response: aiohttp.ClientResponse, stats: RequestStats
    ):
        async with response:
    ...
    def _read_utilization(self, response: aiohttp.ClientResponse, stats: RequestStats):
        if UTILIZATION_HEADER in response.headers:
            ...
            if len(util_str) == 0:
                ...
                logging.warning(
                    f"invalid utilization header value: {UTILIZATION_HEADER}={util_str}"
                )
            else:
                try:
